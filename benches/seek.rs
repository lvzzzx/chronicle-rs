...
fn bench_seek(c: &mut Criterion) {
    ...
    let indexed_path = dir.path().join("bench_queue_indexed");
    let no_index_path = dir.path().join("bench_queue_no_index");
    build_queue(&indexed_path);
    build_queue(&no_index_path);
    remove_index_files(&no_index_path);
    ...
    group.bench_function("seek_seq_mid", |b| {
        b.iter_batched(
            || {
                ...
                Queue::open_subscriber_with_config(&indexed_path, &name, reader_config)
                    .expect("reader")
            },
            |mut reader| {
    ...
    group.bench_function("seek_seq_mid_no_index", |b| {
        b.iter_batched(
            || {
                let id = counter.fetch_add(1, Ordering::Relaxed);
                let name = format!("bench_reader_seq_no_index_{id}");
                Queue::open_subscriber_with_config(&no_index_path, &name, reader_config)
                    .expect("reader")
            },
            |mut reader| {
                reader.seek_seq(TARGET_SEQ).expect("seek");
    ...
    group.bench_function("seek_timestamp_mid_no_index", |b| {
        b.iter_batched(
            || {
                let id = counter.fetch_add(1, Ordering::Relaxed);
                let name = format!("bench_reader_ts_no_index_{id}");
                Queue::open_subscriber_with_config(&no_index_path, &name, reader_config)
                    .expect("reader")
            },
            |mut reader| {
...
fn build_queue(path: &std::path::Path) {
...
fn remove_index_files(path: &std::path::Path) {
    ...
    for entry in entries {
        ...
        if name_str.ends_with(".idx") {
            std::fs::remove_file(entry.path()).expect("remove idx");
        }
    }
}
