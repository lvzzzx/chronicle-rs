...
use chronicle_core::control::ControlBlock;
use chronicle_core::mmap::MmapFile;
use chronicle_core::segment::segment_path;
use chronicle_core::{DisconnectReason, Queue, WriterConfig};
...
fn detect_disconnect_heartbeat_stale() {
    ...
    let control_path = queue_path.join("control.meta");
    let control = chronicle_core::control::ControlFile::open(&control_path).expect("control");
