        ...
        thread::Builder::new()
            ...
            .spawn(move || {
                ...
                loop {
                    ...
                    match crate::core::retention::min_live_reader_position(
                        &path,
                        head_segment,
                        head_offset,
                        segment_size,
    ...
    pub fn open_publisher_with_clock<C: Clock>(
        ...
        if control_segment > segment_id {
            ...
            if prev_path.exists() {
                ...
                if (prev_header.flags & SEG_FLAG_SEALED) == 0 {
                    crate::core::segment::repair_unsealed_tail(&mut prev_mmap, segment_size)?;
                }
            }
        ...
        if !tail_partial && (header.flags & SEG_FLAG_SEALED) != 0 {
            if next_path.exists() {
                ...
                mmap = prepare_segment(&path, next_segment_id as u64, segment_size)?;
                segment_id = next_segment_id;
            }
            write_offset = SEG_DATA_OFFSET as u64;
        }
...
impl<C: Clock> QueueWriter<C> {
        ...
        loop {
            ...
            match self.config.backpressure {
                BackpressurePolicy::FailFast => {
                    ...
                    if let Ok(pos) = crate::core::retention::min_live_reader_position(
                        &self.path,
                        head_segment,
                        head_offset,
    ...
    fn roll_segment(&mut self) -> Result<()> {
        ...
        let new_mmap = if let Some(prepared) = self.acquire_preallocated(next_segment as u64)? {
            ...
            prepare_or_open_segment(&self.path, next_segment as u64, self.segment_size)?
        };
...
fn repair_tail_and_roll(
    ...
    let header = read_segment_header(mmap)?;
    if (header.flags & crate::core::segment::SEG_FLAG_SEALED) != 0 {
        let next_segment = segment_id + 1;
        prepare_segment(root, next_segment as u64, segment_size)?;
        return Ok(Some(next_segment));
    }

    crate::core::segment::repair_unsealed_tail(mmap, segment_size)?;
    let next_segment = segment_id + 1;
    prepare_segment(root, next_segment as u64, segment_size)?;
