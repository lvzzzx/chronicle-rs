...
/// The number of records between index entries.
///
/// A stride of 4096 means the index stores one entry for every 4096 messages.
/// This strikes a balance between index size (sparse) and seek precision.
/// A linear scan of ~2048 messages (average) in a memory-mapped file is extremely fast (nanoseconds/microseconds)
/// compared to the overhead of maintaining a dense index.
