...
- [x] (2026-01-16 13:46Z) Added fast stress test and ignored heavy stress test in `crates/chronicle-core/tests/stress_single_writer.rs`.
- [x] (2026-01-16 13:46Z) Added ignored soak test in `crates/chronicle-core/tests/soak_writer_reader.rs`.
- [x] (2026-01-16 13:46Z) Added Criterion benchmarks in `crates/chronicle-core/benches/append.rs` and `crates/chronicle-core/benches/read.rs`.
- [x] (2026-01-16 13:46Z) Updated `crates/chronicle-core/Cargo.toml` with Criterion dev-dependency and bench harness entries.
...
- Decision: Implement stress and soak tests as integration tests under `crates/chronicle-core/tests`, using `#[ignore]` for long runs and environment variables to tune message counts and durations.
...
The data plane lives in `crates/chronicle-core`. Writers append to memory-mapped segment files via `Queue::open_publisher` and `QueueWriter::append` in `crates/chronicle-core/src/writer.rs`. Readers consume committed messages via `Queue::open_subscriber` and `QueueReader::next` in `crates/chronicle-core/src/reader.rs`. Segments roll at `SEGMENT_SIZE` in `crates/chronicle-core/src/segment.rs`, and retention cleanup is handled by `crates/chronicle-core/src/retention.rs`. Integration tests already live in `crates/chronicle-core/tests` and use temporary directories from `tempfile`.
...
Edit or add the following files:
    ...
    crates/chronicle-core/tests/stress_single_writer.rs
    crates/chronicle-core/tests/soak_writer_reader.rs
    crates/chronicle-core/benches/append.rs
    crates/chronicle-core/benches/read.rs
...
- Dev dependency in `crates/chronicle-core/Cargo.toml`: `criterion = "0.5"` (or the latest compatible 0.x version).
- Bench harness entries in `crates/chronicle-core/Cargo.toml` with `harness = false` for `append` and `read`.
